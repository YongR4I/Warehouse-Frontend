    ...
    <MenuPrimitive.Portal>
      ...
      >
        <MenuPrimitive.Popup
          ...
          className={cn(
            "z-50 max-h-(--available-height) min-w-44 origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-xl border border-border/80 bg-popover p-1.5 text-popover-foreground shadow-md transition-all duration-100 outline-none data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 dark:border-border/40 dark:shadow-black/20 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:overflow-hidden data-closed:fade-out-0 data-closed:zoom-out-95",
            className
          )}
          {...props}
        />
...
function DropdownMenuSubTrigger({
  ...
  return (
    ...
    >
      {children}
      <ChevronRightIcon className="ml-auto text-muted-foreground/60 group-focus/dropdown-menu-sub-trigger:text-foreground/80" />
    </MenuPrimitive.SubmenuTrigger>
  )
}
