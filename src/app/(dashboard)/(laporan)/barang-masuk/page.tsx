...
import type { Gudang, Supplier, LaporanRow } from "@/types"
import {
  BiSolidReport,
  BiBarChartAlt2,
  BiChevronRight,
...
export default function LaporanBarangMasukPage() {
  ...
  const laporanParams = useMemo(
    ...
    [gudangId, supplierId, fromDate, toDate]
  )
  ...
  const filteredData = useMemo(() => {
    ...
    return rawLaporan.filter((row) => {
      return (
        ...
        (row.supplier_nama ?? "").toLowerCase().includes(queryStr) ||
  ...
  const exportUrl = `/laporan/barang-masuk?format=excel&from=${fromDate}&to=${toDate}${
    ...
    <>
      ...
      <div className="wrapper">
        <div className="flex items-end justify-between">
          <PageHeader
            ...
            icon={BiTrendingDown}
            description="Laporan penerimaan barang dari supplier dengan detail transaksi."
          />
            ...
            <Card className="min-h-[114px] w-full transition-all duration-200 hover:-translate-y-1 hover:shadow-md">
              <div className="flex h-full flex-col justify-between px-5 py-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Jumlah keseluruhan barang masuk
                  </p>
                </div>
              </div>
            </Card>
      ...
      <div className="wrapper mt-8">
        <div className="flex flex-wrap items-center gap-2">
          <InputSearch
            placeholder="Cari no. referensi, supplier, atau gudang..."
            className="flex-1"
            value={searchQuery}
            onChange={(e) => {
