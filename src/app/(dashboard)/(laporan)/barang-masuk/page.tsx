...
export default function LaporanBarangMasukPage() {
  ...
  const canView = hasPermission("laporan-barang-masuk")

  const { data, isLoading } = useApiList<BarangMasuk>({
    key: "laporan-barang-masuk",
  ...
  const summaryMetrics: ReportSummaryCardItem[] = useMemo(() => {
    ...
    const uniqueSuppliers = new Set(
      items.map((r) => r.supplier_id || r.supplier?.id).filter(Boolean)
    )

    const approvedCount = items.filter((r) => r.status === "approved").length
    const approvedRate = total > 0 ? (approvedCount / items.length) * 100 : 100
    ...
    return [
      {
        ...
        subLabel: "Dokumen",
        description: `Penerimaan periode ${formatDate(fromDate)} – ${formatDate(toDate)}`,
        icon: FileText,
        badge: {
          text: "Penerimaan",
          color: "blue",
      ...
      {
        title: "Total Qty Barang Masuk",
        value: formatNumber(totalQty),
        subLabel: "Unit",
        description: "Jumlah volume fisik barang yang diterima",
        icon: PackageCheck,
        badge: {
          text: "Akumulasi",
          color: "green",
      ...
      {
        title: "Total Supplier Terlibat",
        value: formatNumber(uniqueSuppliers.size),
        subLabel: "Pemasok",
        description: "Mitra aktif dalam periode laporan",
        icon: Truck,
        badge: {
          text: "Mitra Aktif",
          color: "gray",
      ...
      {
        title: "Tingkat Disetujui (Approved)",
        value: `${approvedRate.toFixed(1)}%`,
        subLabel: `${approvedCount} Selesai`,
        description: "Akurasi status transaksi penerimaan barang",
        icon: CheckCircle2,
        badge: {
          text: approvedRate >= 80 ? "Optimal" : "Perhatian",
          color: approvedRate >= 80 ? "green" : "yellow",
        },
      },
    ]
  ...
  const exportUrl = `/laporan/barang-masuk?${exportQuery.toString()}`
  ...
  if (!canView) {
    return (
      <div className="wrapper">
        <PageHeader
          items={[{ label: "Pusat Laporan" }, { label: "Laporan Barang Masuk" }]}
          title="Laporan Barang Masuk"
          icon={BiDownArrowCircle}
          description="Laporan komprehensif penerimaan barang dari supplier, volume unit, dan status verifikasi."
        />
        <div className="mt-8 flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-16 text-center">
          ...
          <p className="mt-1 max-w-md text-sm text-muted-foreground">
            Akun Anda tidak memiliki izin akses (<code>laporan-barang-masuk</code>) untuk melihat halaman laporan ini.
          </p>
          <Button
            variant="outline"
            className="mt-4"
  ...
  return (
    <>
          ...
          <TableBody className="min-h-[300px]">
            {isLoading ? (
              ...
              <TableRow className="h-16 border-b border-border/40 hover:bg-transparent">
                ...
                >
                  Tidak ada data laporan barang masuk untuk filter yang dipilih.
                </TableCell>
              </TableRow>
            ) : (
              items.map((row) => (
      ...
      <ExportModal
        ...
        title="Ekspor Laporan Barang Masuk"
        ...
        checkboxes={[
          ...
          { id: "tanggal", label: "Tanggal Penerimaan", defaultChecked: true },
          { id: "supplier", label: "Supplier / Pemasok", defaultChecked: true },
          { id: "gudang", label: "Gudang Tujuan", defaultChecked: true },
          { id: "qty", label: "Total Qty Item", defaultChecked: true },
          { id: "status", label: "Status Verifikasi", defaultChecked: true },
        ]}
      />
    </>
  )
